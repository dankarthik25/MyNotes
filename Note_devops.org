...
git git show stast <> # To inspect or check details of particual stash:
...
zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzx*** Configure Jenkins with Tomcat for Auto Deployment of Artifacts.
#+BEGIN_SRC sh
  cd /opt/tomcat/conf
...
** Q) upgrade  database with in container 
#+begin_src sh 
...
For [more information read [[https://docs.docker.com/develop/develop-images/dockerfile_best-practices/][docker offical link]]
** TODO Edit Docker File [Health and info about each instruction] 
#+BEGIN_SRC sh
...
services:  # containers. same as docker run
  <servicenam>e: # a friendly name. this is also DNS name inside network
...
docker-compose up -d
docker-compose up
docker-compose down # stop all container and remove volumes thee volumes
docker-compose ps
docker-compose top 
#Commands:
...
Different containers same same DNS is know as Docker DNS Roundrobin
Ex: google.com
...
docker service create --name worker -p 80:80 --network frontend --replica 2 dockersamples/examplevottingvotle:befo

docker service create --name worker -p 80:80 --network backend --replica 2 dockersamples/examplevottingvotle:befo
...
Note we can see user_id and password by runing =cat /run/secrets/<secret-user_id/password>or<secret-alias>=
...
#dan_psql
#myDd_passwrd
#      docker secret create psd_user psql_user.txt ps
