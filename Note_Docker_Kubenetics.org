...
docker servie create --name result --network backend -p 5001:80
 ...
 #+END_SRC
* Stacks : Production Grade Compose[71]
** Intro
...
* Secrets[72] 
** Intro
- Secrets Storage [1.13.1 version]
  - Easiest('.' build inside swarm) "secure('.' it's encripted and only avaiable when it is needed  )" solution for storing "secure"  in swarm
  - What is secret ?  # which doesn't show in front page or encripted
 ...
 - In Docker Swarm RaftDb is encripted on disk to store secret
 - Only stored on disk on Manager Nodes [Only manager can unlock/decripted secret-key]
 - Who secret are passed arround container/services?
   - secret are 1st stored in swarm, then assigned to services        
   - only containers in assinged service can see them
 - Who they are stored and where they are stored in container/services?
...
#+end_src
** Secret with [services/Swarm][73]  
#+begin_src sh
# create file psql_user, psql_password which contain id and password 
...
** Secret with Stack [74]
Stack compose  consist of services,volumes,networks + deploy but it also contain =secret= feature
There are two way to give =secret= in stack  
- as filbe
  #+begin_src sh
version :"3.1"
...
services:
  psql:
    ...
    environment:
      ...
      POSTGRES_USER_FILE: /run/secretes/psql_user
secrets:
   psql_user:
     file: ./psql_user.txt
   psql_user:
     file: ./psql_password.txt
# docker stack deploy -c stack-compose.yml mydb
# docker stack rm mydb #NOTE: also clean secrets

  #+end_src
- create secret (cli or api ) and pass them in stack compose file.  
where we can give secret file or externally
...
** Assignment: Stack with Secret[75-76]
Create a stack for Drupal  using psql with secrets

- Steps :
  - Compose yml file with drupal without build of drupal and psql
  - use secrete external but not from files like cli
#+begin_src 
version: '3.1'
...
# echo "adfdsfoe1@" | docker secrete create psql-pw -
# docker stack deploy -c stack-compose.yml drupal
# NOTE : Need to remove history for remove password
#+end_src
